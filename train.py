    ...
    def fit(self, train_loader, val_loader, fold: int):
        ...
        for _epoch in range(self.config.n_epochs):
            ...
            train_reported_results = [
                result.report(
                    train_results_computed[result.__class__.__name__])
            ...
            if self.early_stopping is not None:
                best_score, early_stop = self.early_stopping.should_stop(
                    curr_epoch_score=val_results_computed[
                ...
                self.best_val_results[
                    self.config.monitored_result] = best_score
                self.save("{}_best_{}_fold_{}.pt".format(
                    self.config.effnet, self.config.monitored_result, fold))
                if early_stop:
                    break

            # Compute the new best value for all selected ComparableMetric
            # validation results. If we find a new best value for the selected
            # monitored result, save the model.
            for result in self.selected_results_val:
                if not isinstance(result, results.ComparableResult):
                    continue

                old_value = self.best_val_results.get(
                    result.__class__.__name__, None)

                if old_value is None:
                    self.best_val_results[
                        result.__class__.__name__] = old_value

                    if (result.__class__.__name__ ==
                            self.config.monitored_result):
                        self.save(
                            os.path.join(
                                self.save_path,
                                "{}_{}_best_{}_fold_{}.pt".format(
                                    ...
                                    self.config.monitored_result, fold)))
                ...
                if result.compare(old_value, new_value):
                    self.best_val_results[
                        result.__class__.__name__] = new_value
