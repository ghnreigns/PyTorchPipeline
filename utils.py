...
def get_file_type(image_folder_path: str, allowed_extensions: Optional[List] = None):
    ...
    assert len(extension_dict.keys()) == 1, "The extension in the folder should all be the same, "
...
def check_df_ext(df: pd.DataFrame, col_name: str, allowed_extensions: Optional[List] = None):
    ...
    assert len(set(extension_type)) == 1, "The extension in the image id should all be the same"
